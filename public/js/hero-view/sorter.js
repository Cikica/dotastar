define({
	...
	make : function (sort) {
		...
		var sorted, self, library, library_path, component_path
		self         = this
		library_path = this.return_given_module_paths_without_js_postfixes({
			path : sort.library_path
		})
		component_path = this.return_given_module_paths_without_js_postfixes({
			path : sort.path
		})
		...
		window.hero_view = sort.module[0].make(window.hero_view)
		return sorted
	},
	...
	return_given_module_paths_without_js_postfixes : function (module) {
		var self = this
		return this.loop_array({
			into     : [],
			start_at : 0,
			...
			else_do : function (loop) {
				return { 
					into    : loop.into.concat(self.if_given_path_has_js_postfix_remove_it_and_return_result(loop.array[loop.start_at])),
	...
	if_given_path_has_js_postfix_remove_it_and_return_result : function (path) {
		return ( path.match(/\.js$/) !== null ? path.slice(0, path.length-3 ) : path )
	},
